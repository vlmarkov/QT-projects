...
{
    static std::vector<CpuData> cpuDataPrev;
    std::vector<CpuData> cpuDataNext;
    ...
    if (fileStat.is_open())
    {
        ...
        while (std::getline(fileStat, line))
        {
            ...
            if (!line.compare(0, LEN_STR_CPU, STR_CPU))
            {
                ...
                // Stores entry
                cpuDataNext.emplace_back(CpuData());
        ...
        {
            this->cpuUsage_ = std::vector<CpuUsage>(cpuDataNext.size() - SKIP_COMMON_CPU_STAT);
...
Q_INVOKABLE QString CpuMonitor::getCoreName(const int core)
{
    return this->cpuUsage_[core].name;
}

Q_INVOKABLE double CpuMonitor::getCoreUsage(const int core)
{
    return this->cpuUsage_[core].usage;
}

/******************************************************/
/* Private methods                                    */
/******************************************************/
void CpuMonitor::computeUsage_(const std::vector<CpuData>& cpuDataPrev,
